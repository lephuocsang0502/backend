...
// router.put('/updateOrder/:orderId', (req,res,next) => {
//   const {isPaid} = req.body;
//   console.log("order",isPaid,req.params)
//   pool.query('update "Client" SET "isPaid"=$1 where("ID"=$2) ',[isPaid,req.params.orderId] );
//   res.send(`isPaid : ${isPaid} , id : ${req.params.orderId}`)
// })
...
router.put('/updateorder/:ID', function (req, res, next) {
  const {roomnum,TypeClient,CMND,Address,DateAgent,Name,DateSolve,Days,Total,isPaid}=req.body
  console.log(req.body)
    const ID = parseInt(req.params.ID)
    console.log('start');

  pool.query(
    'update "Client" SET roomnum=$1,"TypeClient"=$2, "CMND"=$3, "Address"=$4 ,"DateAgent"=$5,"Name"=$6,"DateSolve"=$7,"Days"=$8,"Total"=$9,"isPaid"=$10 where("ID" = $11)',
     [roomnum,TypeClient,CMND,Address,DateAgent,Name,DateSolve,Days,Total,isPaid,ID],
    (err, response) => {
      console.log(response);
      if (err) {
        console.log("loi",err)
